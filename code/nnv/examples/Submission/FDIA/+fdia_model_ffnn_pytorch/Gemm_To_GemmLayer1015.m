classdef Gemm_To_GemmLayer1015 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Gemm_To_GemmLayer1015(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
        ...
        function [output] = predict(this, input, inputNumDims)
            ...
            if any(cellfun(@(A)~isnumeric(A), {output}))
                fprintf('Runtime error in network. The custom layer ''%s'' output a non-numeric value.\n', 'Gemm_To_GemmLayer1015');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Gemm_To_GemmLayer1015'));
            end
...
%				  INPUT:		[batch_size, 128]				Type: FLOAT
...
[output, outputNumDims, state] = Gemm_To_GemmGraph1006(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[output] = postprocessOutput(output, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [output, outputNumDims1014, state] = Gemm_To_GemmGraph1006(input, inputNumDims1013, Vars, NumDims, Training, state)
% Function implementing the graph 'Gemm_To_GemmGraph1006'
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc1_Gemm_output_0] = prepareGemmArgs(Vars.input, Vars.fc1_weight, Vars.fc1_bias, Vars.Gemmalpha1007, Vars.Gemmbeta1008, 0, 1, NumDims.fc1_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.x_fc2_Gemm_output_0] = prepareGemmArgs(Vars.x_Relu_output_0, Vars.fc2_weight, Vars.fc2_bias, Vars.Gemmalpha1009, Vars.Gemmbeta1010, 0, 1, NumDims.fc2_bias);
...
% Gemm:
[A, B, C, alpha, beta, NumDims.output] = prepareGemmArgs(Vars.x_Relu_1_output_0, Vars.fc3_weight, Vars.fc3_bias, Vars.Gemmalpha1011, Vars.Gemmbeta1012, 0, 1, NumDims.fc3_bias);
Vars.output = alpha*B*A + beta*C;

% Set graph output arguments from Vars and NumDims:
output = Vars.output;
outputNumDims1014 = NumDims.output;
